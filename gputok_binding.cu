...
// Optimized BlockBPE kernel (matches the \"optimized\" standalone binary semantics)
...
// Baseline BlockBPE kernel: mirrors Algorithm 1 as in gputok_blockbpe.cu
__global__ void bpe_merge_kernel_blockbpe_baseline(DevicePairMapRef map_ref,
                                                   const std::int32_t* __restrict__ d_tokens_in,
                                                   const int* __restrict__ d_offsets,
                                                   const int* __restrict__ d_lengths,
                                                   std::int32_t* __restrict__ d_tokens_out,
...
public:
  ...
  // Tokenize with the baseline BlockBPE kernel (greedy Algorithm 1),
  // matching the behavior of the standalone gputok_blockbpe.cu binary.
  std::pair<std::vector<std::vector<std::int32_t>>, double>
  tokenize_batch_blockbpe_base(const std::vector<std::string>& texts)
  {
    if (texts.empty()) {
      return {{}, 0.0};
    }
    ...
    cudaEventRecord(ev_start);
    auto d_map_ref = d_map_->ref(cuco::find);
    bpe_merge_kernel_blockbpe_baseline<<<grid_size, block_size, shared_bytes>>>(
      d_map_ref, d_tokens_in, d_offsets, d_lengths, d_tokens_out, d_out_lengths, max_seq_len);
    cudaEventRecord(ev_end);
    cudaEventSynchronize(ev_end);
    float kernel_ms = 0.0f;
    cudaEventElapsedTime(&kernel_ms, ev_start, ev_end);
    check_cuda(cudaGetLastError(), "bpe_merge_kernel_blockbpe_baseline");
    ...
    // Unflatten into batch of sequences corresponding to input texts
    std::vector<std::vector<std::int32_t>> batch(texts.size());
    for (int i = 0; i < num_seqs; ++i) {
      int owner = seq_owner[i];
      if (owner < 0 || static_cast<std::size_t>(owner) >= batch.size()) {
...
Returns:
  ...
  - kernel_time_ms:  total GPU kernel time for the batch in milliseconds
    ...
    .def("tokenize_batch_blockbpe_base",
         [](GpuTokenizer& self, const std::vector<std::string>& texts) {
           auto result = self.tokenize_batch_blockbpe_base(texts);
           return py::make_tuple(result.first, result.second);
         },
         py::arg("texts"),
         R"pbdoc(
Tokenize a batch of UTF-8 strings using the baseline BlockBPE kernel
(matching the standalone gputok_blockbpe.cu implementation).
